...
단계:
  ...
  대기           : PHASE_WAIT_SEC
  2단계 [회전]  : 제자리 180도 회전 (좌/우 교환 감지 또는 타임아웃)
  대기           : PHASE_WAIT_SEC
  3단계 [귀환]  : 교환된 기준으로 전진 → 전방 장애물 정지

사용법:
  indoor_run_auto.py                    # 기본 파라미터
  indoor_run_auto.py --front-stop 0.6   # 전방 정지 거리
  indoor_run_auto.py --fspeed 0.50      # 전진 속도
  indoor_run_auto.py --scan             # 센서값 확인
"""
...
# ── 단계 전환 대기 ─────────────────────────────────────────────────
PHASE_WAIT_SEC    = 2.0   # s  각 단계 전환 전 정지 유지 시간
...
WALL_SPAN_MIN_DEG     = 40   # 이 각도 이상 → 벽 (정지)  미만 → 기둥 (무시)
...
def rotate_phase(pub, rot_left_before, rot_right_before):
    ...
    rospy.loginfo(
        f"[2단계] 회전 시작  목표: 좌≈{rot_right_before:.2f}m  우≈{rot_left_before:.2f}m"
    )
...
def run(pub, front_stop, fspeed):
    ...
    rospy.loginfo("=" * 50)
    rospy.loginfo("실내 경로 완전 자동 왕복 주행")
    ...
    rospy.loginfo(f"  단계 대기  : {PHASE_WAIT_SEC:.1f}s")
    rospy.loginfo("=" * 50)
    ...
    rospy.loginfo(f"[대기] {PHASE_WAIT_SEC:.1f}s 후 회전 시작...")
    rospy.sleep(PHASE_WAIT_SEC)
    ...
    rospy.loginfo(f"[대기] {PHASE_WAIT_SEC:.1f}s 후 귀환 시작...")
    rospy.sleep(PHASE_WAIT_SEC)
...
def main():
    ...
    if '--scan' in args:
        rospy.init_node('indoor_run_auto_scan', anonymous=True)
    ...
    rospy.init_node('indoor_run_auto', anonymous=True)
