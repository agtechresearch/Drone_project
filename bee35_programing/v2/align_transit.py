...
# -*- coding: utf-8 -*-
"""
[실험군] AprilTag 정렬 + 우 이동 + 도착지 정렬 실험

실험 흐름:
  1. 이륙 (TARGET_ALT_M)
  2. 시작 마커(START_TAG_ID) 정렬       — PID 알고리즘 적용
  3. 시작 위치 호버링 (START_HOVER_S)   — LOITER 단독
  4. 우 이동 (TRANSIT_ROLL, TRANSIT_S)
  5. 도착 마커(GOAL_TAG_ID) 감지 대기   — 최대 GOAL_ACQUIRE_TIMEOUT_S
  6. 도착지 측정 (GOAL_MEASURE_S)       — PID 알고리즘 적용
  7. 착륙

대조군(loiter_transit.py)과 동일한 흐름이며, 6단계에서만 차이가 있음:
  - 실험군: PID 정렬 적용
  - 대조군: 오버라이드 없이 LOITER 단독

실행 순서:
  터미널1: libcamera-vid --width 640 --height 480 --codec mjpeg --framerate 60 -t 0 --listen -o tcp://127.0.0.1:8888
  터미널2: mavproxy.py --master=/dev/ttyAMA0 --baudrate 115200 --out=udp:192.168.45.171:14550 --out=udp:127.0.0.1:14551
  터미널3: python align_transit.py
"""
...
# 실험군 플래그 (대조군에서는 False)
APPLY_PID_AT_GOAL  = True
