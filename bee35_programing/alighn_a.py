...
# -*- coding: utf-8 -*-
"""
드론 자동이륙 + AprilTag 정렬 제어 (첫 정렬 후 ALTHOLD 전환)
- Python 3.9
- libcamera-vid TCP 스트림 + pupil-apriltags + OpenCV
- 카메라 스레드 / 비행 루프 분리
- CH5 안전 스위치
...
# PID 게인
# Roll — 정밀
KP_ROLL  = 0.15   # 낮게, 과보정 방지
KI_ROLL  = 0.005  # 최소한으로
KD_ROLL  = 0.18   # 높게, 미세 진동 억제

# Pitch — 오차 줄이기
KP_PITCH = 0.35   # 높게, 빠른 접근
KI_PITCH = 0.02   # 잔류 오차 보완
KD_PITCH = 0.12   # 응답 빠르게

I_LIMIT  = 20

# 게인 스케일링 — 오차가 클수록 KP를 키움
# 허용범위(ALIGN/PITCH_THRESHOLD) 안쪽: 기본 KP
# 허용범위 밖: KP_SCALE_OUT 배율 적용
KP_SCALE_OUT_ROLL  = 2.5   # 허용범위 밖 roll KP 배율
KP_SCALE_OUT_PITCH = 4.0   # 허용범위 밖 pitch KP 배율 (앞뒤 더 강하게)

# 미탐지 시 마지막 보정값 유지 시간 (초)
# 이 시간 이후에도 탐지 안 되면 보정 0 + 적분 리셋
...
def init_logs():
    ...
    rt_path = f"logs/realtime_{ts}.csv"
    ...
    sum_path = f"logs/summary_{ts}.csv"
...
_pid = {
    ...
    "prev_err_pitch": 0.0,
    "prev_time":      None,
    "last_corr_roll":  0,    # 마지막 보정값 유지용
    "last_corr_pitch": 0,
    "no_detect_since": None, # 미탐지 시작 시각
}


# ─────────────────────────────────────────
# 마커 기반 오버라이드 보정값 계산 (PID)
# ─────────────────────────────────────────
def calc_marker_correction():
    """마커 오차 → roll/pitch 보정값 (μs). 미감지 시 적분 리셋 후 (0,0) 반환.
    허용범위 밖에서는 KP를 KP_SCALE_OUT 배 적용해 더 강하게 복귀.
    """
    with _marker_lock:
    ...
    if not detected:
        # 미탐지 시작 시각 기록
        if _pid["no_detect_since"] is None:
            _pid["no_detect_since"] = time.time()

        elapsed_no_detect = time.time() - _pid["no_detect_since"]

        if elapsed_no_detect < NO_DETECT_HOLD_S:
            # 유지 시간 내: 마지막 보정값 그대로 유지
            return _pid["last_corr_roll"], _pid["last_corr_pitch"]
        else:
            # 유지 시간 초과: 리셋 후 0 반환
            _pid["integral_x"]     = 0.0
            _pid["integral_pitch"] = 0.0
            _pid["prev_time"]      = None
            _pid["last_corr_roll"]  = 0
            _pid["last_corr_pitch"] = 0
            return 0, 0
    else:
        # 탐지 복귀 시 미탐지 타이머 리셋
        _pid["no_detect_since"] = None
    ...
    # ── Roll (err_x → 좌우) ───────────────
    # 허용범위 밖이면 KP 강화
    kp_roll = KP_ROLL * KP_SCALE_OUT_ROLL if abs(err_x) > ALIGN_THRESHOLD_PX else KP_ROLL
    ...
    # ── Pitch (err_pitch → 앞뒤) ──────────
    # 허용범위 밖이면 KP 강화
    kp_pitch = KP_PITCH * KP_SCALE_OUT_PITCH if abs(err_pitch) > PITCH_THRESHOLD_PX else KP_PITCH
    ...
    # 마지막 보정값 저장 (미탐지 시 유지용)
    _pid["last_corr_roll"]  = corr_roll
    _pid["last_corr_pitch"] = corr_pitch
...
def main():
    ...
    try:
        ...
        if not manual_during_stabilize:
            print("[HOVER] 호버링 시작 — AprilTag 정렬 중...")
            print("        CH5 스위치로 수동 전환 가능")

            log_start         = time.time()  # elapsed_s 기준 (절대, 리셋 안 함)
            segment_start     = time.time()  # 구간 누적 기준 (정렬 이탈 시 리셋)
            attempt           = 0
            was_aligned       = False
            first_align_time  = None   # 첫 정렬 시각
            RECORD_DURATION   = 60.0   # 첫 정렬 후 동작 시간 (초)
            ...
            while True:
                ...
                if aligned and not was_aligned:
                    ...
                    sum_file.flush()
                    print(f"\n[LOG] #{attempt} 정렬 완료 — 소요시간:{align_time:.2f}s "
                          f"누적오차 x:{sum_abs_err_x} pitch:{sum_abs_err_pitch} rms_vib:{rms_vib:.4f}")

                    # 첫 정렬이면 ALTHOLD 전환 + 타이머 시작
                    if first_align_time is None:
                        first_align_time = time.time()
                        print(f"[TIMER] 첫 정렬 감지 — ALTHOLD 전환 후 {RECORD_DURATION:.0f}초 후 착륙")
                        vehicle.mode = VehicleMode("ALT_HOLD")
                        if not wait_mode(vehicle, "ALT_HOLD", timeout=3.0):
                            print("[WARN] ALTHOLD 전환 실패 — LOITER 유지")
                        else:
                            print("[MODE] ALTHOLD 전환 완료")
                ...
                if detected:
                    status = "ALIGNED ✓" if aligned else f"err_x={err_x:+d} pitch_err={err_pitch:+d}"
                    print(f"[ALIGN] {status} | roll={roll_override} pitch={pitch_override}",
                          end="\r")
                else:
                    print("[ALIGN] 마커 미감지 — 제자리 호버링", end="\r")
