...
# -*- coding: utf-8 -*-
"""
드론 자동이륙 + AprilTag 정렬 (poster — CIGR EurAgEng 2026 본 실험)

핵심 설정 (v16 정착):
  - KP_PITCH = 0.80  (거리 PID, deadband 우회)
  - KP_ROLL  = 0.25  (좌우 PID)
  - ALIGN_GAIN_SCALE = 0.8  (정렬 후 약화, 유지력 보존)
  - TARGET_ALT_M = 1.3m  (마커 1.5m, 안전 고도)

진화 과정:
  - v14 KP_PITCH=0.25:  거리 67cm, 정렬률 29% (deadband 갇힘)
  - v15 KP_PITCH=0.80:  거리 60cm, 정렬률 36%
  - v16 + ALIGN_SCALE 0.8: 거리 54cm, 정렬률 50% (4회 평균) ⭐
  - v17 ALIGN_SCALE 분리: 큰 차이 없음 (노이즈 수준)
  → v16 정착

환경:
  실내 딸기 농장, 재배대 간격 1m, 마커 1.5m 부착
  마커 30mm × 30mm, 운용 거리 25~55cm
  좌우는 가용의 70% 동적 임계
...
로그: logs/poster/loiter_poster_realtime_*.csv

실행:
  python alighn_l_poster.py
"""
...
KI_ROLL  = 0.005
KD_ROLL  = 0.12
KP_PITCH = 0.80      # 0.25 → 0.80 (compensate 우회 + 강한 끌어오기)
KI_PITCH = 0.010
KD_PITCH = 0.12
I_LIMIT  = 20
...
ALIGN_GAIN_SCALE = 0.8       # 0.6 → 0.8 (정렬 후 유지력 강화)
...
def init_logs():
    ...
    rt_path  = f"{log_dir}/loiter_poster_realtime_{ts}.csv"
    sum_path = f"{log_dir}/loiter_poster_summary_{ts}.csv"
...
def camera_thread_fn():
    ...
    log_dir = f"logs/{LOG_SUBDIR}"
    os.makedirs(log_dir, exist_ok=True)
    video_path = f"{log_dir}/flight_video_v16_{ts_str}.mp4"
    ts_path    = f"{log_dir}/flight_video_v16_{ts_str}_timestamps.csv"
...
def main():
    ...
    print(f"  목표 마커 px: {TARGET_MARKER_PX}")
    print(f"  좌우 임계: 가용 좌우의 {LATERAL_RATIO*100:.0f}%")
    print(f"  검출: quad_decimate=2.0, sigma=0.0 (경량)")
    print("=" * 60)
