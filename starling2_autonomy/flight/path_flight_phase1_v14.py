#!/usr/bin/env python3
"""
Starling 2 - Custom Mission Test v14
MAVSDK command + PX4/VOXL pipe based reach judgement + editable mission plan

v14 변경점 (v13 대비):
  - 지오펜스 복구. v13은 MAX_HORIZONTAL_DEV / MAX_ALT_DEVIATION 상수와
    --max-horizontal-dev 인자만 남아 있고 이를 실제로 감시하는 코드가 없었다
    (CR/path_flight_phase1_1.py에는 있었으나 v13으로 오면서 누락된 회귀).
  - 단, CR은 "홈 기준 반경"으로 검사했다. 지금 MISSION_PLAN은 홈에서 10m 넘게
    이동하므로 그대로 이식하면 이륙 직후 오발동한다. 그래서 주 감시 기준을
    "명령 setpoint 대비 추종 오차"로 바꿨다. 미션 크기와 무관하게 동작한다.
  - 홈 기준 절대 펜스는 --max-home-dist / --max-alt-agl 로 따로 켤 수 있다(기본 꺼짐).
  - 비행 로직 자체는 v13과 동일하다. 감시만 추가했다.
...
# ---- Geofence (v14) ----
# 위반이 이 시간 이상 연속으로 지속돼야 발동한다. 단발 VIO 글리치로 착륙시키지 않기 위함.
DEFAULT_GEOFENCE_VIOLATION_SEC = 1.0
# setpoint가 한 샘플에 이 거리 이상 튀면 "즉시 이동 명령"으로 보고 추종 오차 감시를 잠시 유예한다.
# ramp 이동은 20Hz에서 샘플당 수 cm씩만 움직이므로 여기 걸리지 않는다.
DEFAULT_GEOFENCE_JUMP_THRESH = 0.5
# 점프 후 이 시간 안에 새 setpoint로 수렴하지 못하면 그 자체를 고장으로 보고 발동한다.
DEFAULT_GEOFENCE_JUMP_TIMEOUT = 30.0
# 홈 기준 절대 펜스. 0이면 비활성 (미션이 홈에서 10m 이상 가므로 기본은 꺼둔다).
DEFAULT_MAX_HOME_DIST = 0.0
DEFAULT_MAX_ALT_AGL = 0.0
...
class DroneController:
    def __init__(self, args):
        ...
        # ---- Geofence state (v14) ----
        # offboard가 실제로 시작된 뒤에만 켠다. 프리플라이트 중에는 감시하지 않는다.
        self.geofence_armed = False
    ...
    def _geofence_check(self):
        """위반 사유 문자열을 반환한다. 펜스 안이면 None.

        두 겹으로 본다.
          1) 추종 오차 펜스 (주 방어선, 항상 켜짐)
             실제 위치가 명령 setpoint에서 얼마나 벌어졌는가. 미션이 홈에서
             얼마나 멀리 가든 무관하게 "위치 폭주"만 잡아낸다.
          2) 홈 기준 절대 펜스 (기본 꺼짐)
             운용 구역 자체를 벗어났는가. --max-home-dist / --max-alt-agl로 켠다.
        """
        if self.args.no_geofence or not self.geofence_armed:
            return None

        now = asyncio.get_event_loop().time()
        tgt = self.target
        tgt_key = (tgt.north_m, tgt.east_m, tgt.down_m)

        # speed 미지정 이동은 setpoint가 목적지로 즉시 점프한다. 이때 추종 오차가
        # 커지는 것은 정상이므로, 새 setpoint에 수렴할 때까지 1)만 유예한다.
        if self._gf_last_target is not None:
            jump = math.sqrt(
                (tgt_key[0] - self._gf_last_target[0]) ** 2
                + (tgt_key[1] - self._gf_last_target[1]) ** 2
                + (tgt_key[2] - self._gf_last_target[2]) ** 2)
            if jump > self.args.geofence_jump_thresh:
                self._gf_jump_pending = True
                self._gf_jump_since = now
                self._gf_violation_since = None
        self._gf_last_target = tgt_key

        max_h = self.args.max_horizontal_dev
        max_v = self.args.max_alt_deviation
        h_dev = math.sqrt((self.mav_x - tgt.north_m) ** 2 + (self.mav_y - tgt.east_m) ** 2)
        v_dev = abs(self.mav_z - tgt.down_m)

        track_reason = None
        if h_dev > max_h:
            track_reason = "tracking horizontal {:.2f}m > {:.2f}m".format(h_dev, max_h)
        elif v_dev > max_v:
            track_reason = "tracking altitude {:.2f}m > {:.2f}m".format(v_dev, max_v)

        if self._gf_jump_pending:
            if track_reason is None:
                self._gf_jump_pending = False          # 새 setpoint로 수렴 완료
            elif now - self._gf_jump_since > self.args.geofence_jump_timeout:
                return ("GEOFENCE: failed to converge within {:.0f}s after setpoint jump "
                        "({})".format(self.args.geofence_jump_timeout, track_reason))
            else:
                track_reason = None                    # 유예 중

        # 절대 펜스는 유예 중에도 계속 감시한다.
        abs_reason = None
        if self.args.max_home_dist > 0.0:
            home_dist = math.sqrt((self.mav_x - self.initial_x) ** 2
                                  + (self.mav_y - self.initial_y) ** 2)
            if home_dist > self.args.max_home_dist:
                abs_reason = "home distance {:.2f}m > {:.2f}m".format(
                    home_dist, self.args.max_home_dist)
        if abs_reason is None and self.args.max_alt_agl > 0.0:
            agl = self.initial_z - self.mav_z          # NED: down이 줄면 상승
            if agl > self.args.max_alt_agl:
                abs_reason = "altitude AGL {:.2f}m > {:.2f}m".format(agl, self.args.max_alt_agl)

        reason = track_reason or abs_reason
        if reason is None:
            self._gf_violation_since = None
            return None

        # 단발 튐으로 착륙시키지 않도록 일정 시간 연속 위반일 때만 발동한다.
        if self._gf_violation_since is None:
            self._gf_violation_since = now
            log("       [geofence] WARN {} (must persist {:.1f}s to trigger)".format(
                reason, self.args.geofence_violation_sec))
            return None
        if now - self._gf_violation_since < self.args.geofence_violation_sec:
            return None
        return "GEOFENCE: {} sustained {:.1f}s".format(
            reason, self.args.geofence_violation_sec)
    ...
    async def mavsdk_position_monitor(self):
        async for pos in self.drone.telemetry.position_velocity_ned():
            ...
            gf_reason = self._geofence_check()
            if gf_reason:
                asyncio.ensure_future(self.trigger_emergency_land(gf_reason))
    ...
    async def arm_and_start_offboard(self):
        ...
        try:
            ...
            log("       Offboard active!")
            # offboard가 실제로 붙은 다음에만 지오펜스를 켠다.
            self._gf_last_target = (self.target.north_m, self.target.east_m, self.target.down_m)
            self._gf_violation_since = None
            self._gf_jump_pending = False
            self.geofence_armed = not self.args.no_geofence
            if self.geofence_armed:
                log("       [geofence] ARMED: track h<={:.2f}m v<={:.2f}m, "
                    "persist {:.1f}s, home_dist<={}, alt_agl<={}".format(
                        self.args.max_horizontal_dev, self.args.max_alt_deviation,
                        self.args.geofence_violation_sec,
                        "{:.2f}m".format(self.args.max_home_dist) if self.args.max_home_dist > 0 else "off",
                        "{:.2f}m".format(self.args.max_alt_agl) if self.args.max_alt_agl > 0 else "off"))
            else:
                log("       [geofence] DISABLED by --no-geofence")
    ...
    async def land_and_disarm(self):
        log("[7/7] Landing...")
        # 정상 착륙도 offboard setpoint를 따르지 않으므로 펜스를 먼저 끈다.
        self.geofence_armed = False
        if not self.aborted:
            self.streaming = False
            await asyncio.sleep(0.2)
    ...
    async def trigger_emergency_land(self, reason="unknown"):
        ...
        # 착륙 중에는 setpoint 추종 오차가 당연히 커지므로 펜스를 끈다.
        self.geofence_armed = False
...
def build_arg_parser():
    ...
    # ---- Geofence (v14). v13에서는 --max-horizontal-dev가 미배선 상태였다. ----
    p.add_argument("--max-horizontal-dev", type=float, default=MAX_HORIZONTAL_DEV,
                   help="Geofence: max horizontal error between actual position and the commanded "
                        "setpoint, in meters. Exceeding it triggers emergency land.")
    p.add_argument("--max-alt-deviation", type=float, default=MAX_ALT_DEVIATION,
                   help="Geofence: max vertical error against the commanded setpoint, in meters.")
    p.add_argument("--geofence-violation-sec", type=float, default=DEFAULT_GEOFENCE_VIOLATION_SEC,
                   help="Geofence: violation must persist this long before triggering. "
                        "Prevents single-sample VIO glitches from landing the drone.")
    p.add_argument("--geofence-jump-thresh", type=float, default=DEFAULT_GEOFENCE_JUMP_THRESH,
                   help="Geofence: setpoint move larger than this in one sample counts as an "
                        "instant jump, which suspends the tracking fence until it converges.")
    p.add_argument("--geofence-jump-timeout", type=float, default=DEFAULT_GEOFENCE_JUMP_TIMEOUT,
                   help="Geofence: if the drone does not converge to a jumped setpoint within "
                        "this long, treat that as a fault and trigger.")
    p.add_argument("--max-home-dist", type=float, default=DEFAULT_MAX_HOME_DIST,
                   help="Geofence: absolute horizontal radius from takeoff point, in meters. "
                        "0 disables. Note the default MISSION_PLAN travels 10m+ from home.")
    p.add_argument("--max-alt-agl", type=float, default=DEFAULT_MAX_ALT_AGL,
                   help="Geofence: absolute height above takeoff point, in meters. 0 disables.")
    p.add_argument("--no-geofence", action="store_true",
                   help="Disable all geofence monitoring (v13 behaviour).")
